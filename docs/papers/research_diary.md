...
把scannetpp数据集加上去了。并且在采用先用教师模型生成pseudo label，再用学生训练的模式。

把special attention的attn_mask给加上去了（decoder中agg17和agg23的特殊attention mask），但是用在torch上训练会增加显存开销和和增加训练时长。

改成flex_attention似乎没调成功。

或许也可以直接在Attention层，直接提取并减少kv的维度？这样就不需要再构建attn_mask了：
- agg17每一帧的token都只能看见condition frame和前后帧。如果同一个sample每一帧的condition frame都是同一帧，那可以把其他kv全丢弃。
- agg23所有token都只能看见头+尾几帧。可以把其他kv全丢弃。
- 不过以上这种做法，无法让每一帧同时只看到自己帧。


### 0602

- [OrthoReg](https://www.bilibili.com/video/BV1HmduBqEWh)

  预训练模型融合(https://zhuanlan.zhihu.com/p/677088727)，任务正交正则化。

- [RAE v2](https://www.bilibili.com/video/BV1VSV961Eq5)

  谢赛宁的RAE第二版。

  预训练ViT的特征天然有层级分布（dino-v2），深层全局语义，浅层局部语义。融合方法：1. 直接将编码器最后K层的特征逐元素相加。2. 拼接最后K层特征后再降维投影。


### 0623

- [Anchor3R](https://arxiv.org/abs/2606.05035)

  长序列visual mapping，流式3d重建。基于VGGT网络，在32张A800训练12天，用了16个数据集。

  1. sliding window形式，并且以当前帧为锚点（世界坐标系原点）来估计位姿和几何

  2. kv-cache，队列存储kv token，只保存patch token相关的kv。（因为pose query是每一个窗口都在变化的）

  3. 位姿图：sliding window每算一次，就在window内部连一次边。在线更新：每一帧的位姿取李代数中值。离线更新：在完成流式扫描后/检测到闭环的时候，进行全局优化（IRLS算旋转+ADMM算平移）

- [SCOPE](https://arxiv.org/abs/2606.21300)

  序列重建，scale-consistent，基于MoGe继续改进。


### 0428

Point Odyssey的image size是540, 960，其他数据集不是，如果图像过大，会爆显存。。。


### 0430

8080: VDPM + causal attention in aggregator
8081: 原版VDPM
8082: VDPM + causal attention + special attention



### 0505

测评计算开销和时间效率，在lab04卡0上：


- 5 frames VDPM:
  aggregator (1×): mean=163.724 ms  std=0.585 ms
  decoder total (5×): mean=749.542 ms  std=1.607 ms
  decoder per call (mean over repeats): mean=149.908 ms  std=0.321 ms
  point_head total (5×): mean=98.955 ms  std=0.403 ms
  point_head per call (mean over repeats): mean=19.791 ms  std=0.081 ms
  camera_head (1×): mean=7.614 ms  std=0.048 ms
  wall (agg + all dec + all ph + cam): mean=1020.089 ms  std=1.665 ms
  peak torch.cuda.max_memory_allocated: 9.040 GiB
  peak torch.cuda.max_memory_reserved:  10.773 GiB

- 5 frames StreamDPM:
  aggregator:
    total (sum over 5 frames, one sequence): mean=290.007 ms  std=1.243 ms
    mean per frame: 58.001 ms
    last frame (t=4): mean=62.986 ms  std=0.469 ms
  decoder (full prefix, internal cond_tokens):
    total (sum over 5 frames, one sequence): mean=442.787 ms  std=1.267 ms
    mean per frame: 88.557 ms
    last frame (t=4): mean=148.114 ms  std=0.628 ms
  camera_head:
    total (sum over 5 frames, one sequence): mean=24.782 ms  std=0.932 ms
    mean per frame: 4.956 ms
    last frame (t=4): mean=4.925 ms  std=0.024 ms
  point_head:
    total (sum over 5 frames, one sequence): mean=66.092 ms  std=0.509 ms
    mean per frame: 13.218 ms
    last frame (t=4): mean=19.634 ms  std=0.188 ms
  wall (per-frame segment):
    total (sum over 5 frames, one sequence): mean=830.093 ms  std=1.705 ms
    mean per frame: 166.019 ms
    last frame (t=4): mean=237.391 ms  std=0.763 ms
...
- 10 frames VDPM:
  ...
  decoder per call (mean over repeats): mean=327.770 ms  std=1.279 ms
  point_head total (10×): mean=394.096 ms  std=1.624 ms
  point_head per call (mean over repeats): mean=39.410 ms  std=0.162 ms
  camera_head (1×): mean=8.953 ms  std=0.095 ms
  wall (agg + all dec + all ph + cam): mean=4056.485 ms  std=16.218 ms
  peak torch.cuda.max_memory_allocated: 11.214 GiB
  peak torch.cuda.max_memory_reserved:  13.615 GiB

- 10 frames StreamDPM:
  aggregator:
    total (sum over 10 frames, one sequence): mean=651.181 ms  std=1.579 ms
    mean per frame: 65.118 ms
    last frame (t=9): mean=77.460 ms  std=0.257 ms
  decoder (full prefix, internal cond_tokens):
    total (sum over 10 frames, one sequence): mean=1696.849 ms  std=5.107 ms
    mean per frame: 169.685 ms
    last frame (t=9): mean=324.695 ms  std=1.373 ms
  camera_head:
    total (sum over 10 frames, one sequence): mean=49.604 ms  std=0.112 ms
    mean per frame: 4.960 ms
    last frame (t=9): mean=5.021 ms  std=0.051 ms
  point_head:
    total (sum over 10 frames, one sequence): mean=221.057 ms  std=0.751 ms
    mean per frame: 22.106 ms
    last frame (t=9): mean=39.159 ms  std=0.186 ms
  wall (per-frame segment):
    total (sum over 10 frames, one sequence): mean=2637.261 ms  std=6.287 ms
    mean per frame: 263.726 ms
    last frame (t=9): mean=449.250 ms  std=1.467 ms
  peak torch.cuda.max_memory_allocated: 12.960 GiB
  peak torch.cuda.max_memory_reserved:  15.857 GiB

- 15 frames VDPM:
  aggregator (1×): mean=662.805 ms  std=1.845 ms
  decoder total (15×): mean=8120.121 ms  std=15.060 ms
  decoder per call (mean over repeats): mean=541.341 ms  std=1.004 ms
  point_head total (15×): mean=845.202 ms  std=1.594 ms
  point_head per call (mean over repeats): mean=56.347 ms  std=0.106 ms
  camera_head (1×): mean=9.236 ms  std=0.058 ms
  wall (agg + all dec + all ph + cam): mean=9638.028 ms  std=18.098 ms
  peak torch.cuda.max_memory_allocated: 13.481 GiB
  peak torch.cuda.max_memory_reserved:  17.098 GiB

- 15 frames StreamDPM:
  aggregator:
    total (sum over 15 frames, one sequence): mean=1086.558 ms  std=3.071 ms
    mean per frame: 72.437 ms
    last frame (t=14): mean=91.886 ms  std=1.231 ms
  decoder (full prefix, internal cond_tokens):
    total (sum over 15 frames, one sequence): mean=3958.628 ms  std=16.380 ms
    mean per frame: 263.909 ms
    last frame (t=14): mean=538.802 ms  std=3.093 ms
  camera_head:
    total (sum over 15 frames, one sequence): mean=76.756 ms  std=0.853 ms
    mean per frame: 5.117 ms
    last frame (t=14): mean=5.188 ms  std=0.156 ms
  point_head:
    total (sum over 15 frames, one sequence): mean=469.387 ms  std=1.626 ms
    mean per frame: 31.292 ms
    last frame (t=14): mean=56.172 ms  std=0.302 ms
  wall (per-frame segment):
    total (sum over 15 frames, one sequence): mean=5628.286 ms  std=19.905 ms
    mean per frame: 375.219 ms
    last frame (t=14): mean=696.186 ms  std=3.176 ms
  peak torch.cuda.max_memory_allocated: 15.296 GiB
  peak torch.cuda.max_memory_reserved:  24.307 GiB

- 24 frames VDPM:
  aggregator (1×): mean=1309.393 ms  std=1.532 ms
  decoder total (24×): mean=24053.326 ms  std=20.767 ms
  decoder per call (mean over repeats): mean=1002.222 ms  std=0.865 ms
  point_head total (24×): mean=2142.302 ms  std=3.144 ms
  point_head per call (mean over repeats): mean=89.263 ms  std=0.131 ms
  camera_head (1×): mean=7.793 ms  std=0.058 ms
  wall (agg + all dec + all ph + cam): mean=27513.891 ms  std=24.888 ms
  peak torch.cuda.max_memory_allocated: 19.180 GiB
  peak torch.cuda.max_memory_reserved:  25.496 GiB

- 24 frames StreamDPM:
  aggregator:
    total (sum over 24 frames, one sequence): mean=2033.681 ms  std=10.869 ms
    mean per frame: 84.737 ms
    last frame (t=23): mean=115.102 ms  std=0.425 ms
  decoder (full prefix, internal cond_tokens):
    total (sum over 24 frames, one sequence): mean=11094.195 ms  std=24.992 ms
    mean per frame: 462.258 ms
    last frame (t=23): mean=1000.606 ms  std=2.107 ms
  camera_head:
    total (sum over 24 frames, one sequence): mean=123.127 ms  std=0.408 ms
    mean per frame: 5.130 ms
    last frame (t=23): mean=5.239 ms  std=0.169 ms
  point_head:
    total (sum over 24 frames, one sequence): mean=1148.537 ms  std=5.730 ms
    mean per frame: 47.856 ms
    last frame (t=23): mean=88.934 ms  std=0.465 ms
  wall (per-frame segment):
    total (sum over 24 frames, one sequence): mean=14484.779 ms  std=27.839 ms
    mean per frame: 603.532 ms
    last frame (t=23): mean=1216.233 ms  std=2.320 ms
  peak torch.cuda.max_memory_allocated: 20.849 GiB
  peak torch.cuda.max_memory_reserved:  29.705 GiB

- 32 frames VDPM:
  aggregator (1×): mean=2097.570 ms  std=1.355 ms
  decoder total (32×): mean=48665.138 ms  std=20.665 ms
  decoder per call (mean over repeats): mean=1520.786 ms  std=0.646 ms
  point_head total (32×): mean=3817.640 ms  std=1.970 ms
  point_head per call (mean over repeats): mean=119.301 ms  std=0.062 ms
  camera_head (1×): mean=8.551 ms  std=0.050 ms
  wall (agg + all dec + all ph + cam): mean=54590.460 ms  std=21.122 ms
  peak torch.cuda.max_memory_allocated: 26.300 GiB
  peak torch.cuda.max_memory_reserved:  35.203 GiB

- 32 frames StreamDPM:
  aggregator:
    total (sum over 32 frames, one sequence): mean=3036.046 ms  std=4.167 ms
    mean per frame: 94.876 ms
    last frame (t=31): mean=135.402 ms  std=0.184 ms
  decoder (full prefix, internal cond_tokens):
    total (sum over 32 frames, one sequence): mean=21488.809 ms  std=29.344 ms
    mean per frame: 671.525 ms
    last frame (t=31): mean=1518.450 ms  std=2.755 ms
  camera_head:
    total (sum over 32 frames, one sequence): mean=163.719 ms  std=4.972 ms
    mean per frame: 5.116 ms
    last frame (t=31): mean=5.134 ms  std=0.181 ms
  point_head:
    total (sum over 32 frames, one sequence): mean=2002.326 ms  std=4.354 ms
    mean per frame: 62.573 ms
    last frame (t=31): mean=118.834 ms  std=0.633 ms
  wall (per-frame segment):
    total (sum over 32 frames, one sequence): mean=26836.013 ms  std=34.142 ms
    mean per frame: 838.625 ms
    last frame (t=31): mean=1786.123 ms  std=3.308 ms
  peak torch.cuda.max_memory_allocated: 27.521 GiB
  peak torch.cuda.max_memory_reserved:  37.432 GiB


### 0715

TODO 关于PointOdyssey合成数据集的渲染管线:
- [x] outdoors环境的一堆add-on，好像成功运行了
  - [x] HDRI Haven: 下了个blender 4.5的add-on，不过data/hdri里已经有一些例子了，同时去[官网](polyhaven.com/hdris)手动下载了部分hdri场景，总计25个。
  - [x] AMASS: 下了个CMU的子集
  - [x] Mannequin Challenge
  - [x] PartNet: 下载完了，但是如何放在data/里还得看看 (https://github.com/kevin-kaixu/partnet-symh)
  - [x] GSO: data/GSO 里自带一些(https://app.gazebosim.org/GoogleResearch/fuel/collections/Scanned%20Objects%20by%20Google%20Research)，另外下载了总计999个物品。
