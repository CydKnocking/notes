...
=======
我回退了一下关于使用flex_attention的部分,我不想用这部分了,所以改了回去.现在,我发现可以在Decoder中处理special attention时,直接深入到对应的Attention模块里去减少k和v的token,只留下那些"我们想让attention看到的k和v".现在我们可以这么改:具体而言, 1. 对于aggregator第17层的特征(group2的特征),我们希望每个token只看到condition time的帧及前后几帧(frame_d帧),(现在我不需要所有token同时能看到自己帧里其他token了),同时现在在dataset的getitem定义中,我们已经强制同一个sample里的每一帧的view_idxs都一样(也就是说同一个sample里所有帧对应的time condition都是同一帧),并且我们只让batch size=1,所以在group2计算Attention时,可以直接在计算attention score或者F.scaled_dot_product_attention之前,只保留当前这个sample的view_idxs所对应的帧和前后帧的k v token,把其他的k和v去掉就行,不再需要attn_mask; 2. 对于aggregator第23层的特征(group3的特征),我们希望所有token只看到最前+最后的几帧(frame_d帧),(现在我不需要所有token同时能看到自己帧里其他token了),所以同样的,在它对应的Attention里在计算attention score或者F.scaled_dot_product_attention之前,对于k token和v token只需要保留最前+最后几帧对应的k和v token,把其他的k和v去掉即可,不再需要构建attn_mask.   请帮我根据这个思路,阅读相关代码,并添加+修改相关代码
