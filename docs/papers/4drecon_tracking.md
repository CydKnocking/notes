...
- [FrameVGGT](https://arxiv.org/abs/2603.07690)
  ...
  1. 不用token级别的剪枝，而将每一帧的kv-cache作为完整的块进行保留/丢弃，能够保证局部几何结构。（类似关键帧）
...
- [Déjà View](https://arxiv.org/abs/2605.30215)

  循环transformer层。作者发现已有的深度模型中的连续层实际上在重复执行相似的操作，进行几何形状的逐步优化，因此提出与其堆叠几百层不同的参数，不如反复使用同一个共享参数的 Transformer 块来进行迭代优化。

  用了29个数据集，涵盖合成/真实、室内/室外、多视图物体、自动驾驶场景等。用了dino-v2并微调，其他的从头开始训。128张H100。

  推理阶段输入24张图，峰值gpu mem 5GB。用1.17亿参数达到了其他模型10亿参数的效果。

  推理时，循环Transformer层的步数可选。
