...
## 19. 机械臂数据专题：RGB-D、几何与物体/机械臂轨迹（2026-09-22）

### 19.1 先给选择结论：哪些真正适合“重建 + 跟踪”

**有，而且可以组成一套很有针对性的实验，但不能把“有机械臂动作”直接等同于“有物体 3D 轨迹真值”。** 本节优先覆盖 2023–2026 年资源，并保留 CALVIN、HOPE-Video 两个较早但标注有用的参照。下面的建议是结合我们单目 RGB、未知相机位姿、流式长视频设置作出的研究判断：

- **真实数据、深度与机械臂状态：优先 DROID 和 RH20T。** 前者生态与衍生标注更丰富；后者的 RGB-D、机器人状态和标定更直接适合几何分析。两者原始公开格式都不能直接视为通用物体轨迹真值集。
- **真实操作的现成 3D 点轨迹：优先检查 PointWorld-DROID；跟进 TAPVid-MV 的 DROID 子集。** 前者是模型生成的伪标签；后者重点是静态背景和机械臂表面，且当前下载状态仍需等待/确认。两者不能不加区分地当作被操作物体的独立真值。
- **物体位姿、机械臂状态、深度可以共同受控：优先 CALVIN、ManiSkill 和 RoboTwin 2.0。** 适合先验证相机运动与物体运动混淆、遮挡后身份切换等问题；部分标注需要重放或额外导出。
- **真正的分钟级双臂长序列：优先 BEHAVIOR 2026 Challenge。** 它比短时抓取片段更贴近长期漂移和状态保持，但完整资源较大，应从少量任务开始。
- **多视角动态几何、软物体：关注 Robo360。** 它的拍摄系统很有价值，但必须先核对公开子集是否包含所需深度与机器人状态，而不是只依据论文的采集描述。

以下区分四种不同监督：**①深度/相机标定；②关节与末端执行器轨迹；③物体 6D 位姿；④具有持续身份的 2D/3D 表面点轨迹。** 深度只描述可见表面；物体 6D 位姿只约束刚体整体；二者均不自动提供遮挡期间的物质点轨迹。腕部相机是机器人第一人称视角；固定外部相机是第三人称视角；头部相机则通常随移动机器人一起运动。

### 19.2 真实机械臂数据：采集模态、轨迹类型与公开边界

| 数据集 / 年份 | 视角与几何信息 | 机械臂轨迹 | 物体 / 点轨迹与使用限制 |
|---|---|---|---|
| **DROID，2024；2025 标定更新** | 两台外部 ZED 2 双目相机 + 一台腕部 ZED Mini；有内外参。原始立体数据可用于恢复深度，**不能假定常用 RLDS 包已经包含深度图**。 | Franka 关节、末端笛卡尔状态、夹爪与动作；可结合标定获得腕部相机运动。 | 原始格式没有通用物体 6D / 稠密 3D 点身份标注。约 7.6 万演示、350 小时，适合真实场景泛化和操作前后长期一致性。见[数据说明](https://droid-dataset.github.io/droid/the-droid-dataset)、[官方加载与版本说明](https://github.com/droid-dataset/droid_policy_learning)。 |
| **RH20T，2023 起** | 多配置机器人；论文系统包括 1–2 个手上视角、8–10 个全局 RGB-D 视角，提供相机到机器人基座的标定。具体配置需逐条核查。 | 关节角/力矩、末端位姿、夹爪及部分力觉信息；接口提供时间对齐读取。 | 原始发布不等于提供每个被操作物体的 6D 或稠密点轨迹。接触丰富，适合检验接触附近几何误差、夹爪遮挡和视角切换。见[项目与下载说明](https://rh20t.github.io/)、[数据 API](https://github.com/rh20t/rh20t_api)、[论文](https://arxiv.org/abs/2307.00595)。 |
| **Robo360，2023** | 86 路已标定 DSLR RGB + **另外 3 台 RGB-D 相机**；固定第三人称多视角。不能写成 86 路 RGB-D。 | 采集系统记录 xArm6 关节位置、速度、加速度、力矩与控制信息，可求末端位姿。 | 覆盖刚体、布料、绳索等复杂操作；没有核实通用物体 6D / 材料点 GT。论文报告 2,000+ 轨迹、100+ 物体，**不代表全部模态已在公开包中发布**。见[论文](https://arxiv.org/abs/2312.06686)、[公开数据卡](https://huggingface.co/datasets/liuyubian/Robo360)。 |
| **RoboSet，2023** | 左、右、顶部、腕部四路 RGB-D；桌面与厨房操作。 | HDF5 中包含机器人关节、末端、夹爪状态和控制等；公开遥操作部分为 9,500 条轨迹，采样率 5 Hz。 | 适合多阶段操作和多视角几何；未核实物体 6D / 稠密点 GT 与完整标定覆盖。5 Hz 不适合作为高速精细运动跟踪的唯一测试。见[遥操作数据格式与下载](https://robopen.github.io/roboset/teleoperation.html)。 |
| **AgiBot World 2026，2026** | 真实 G2 双臂操作；常见头部及左右手相机。数据格式列出 `observation.images.head_depth`，但需以每个任务的 `meta/info.json` 判断实际模态；不能假定每路相机都有深度。 | LeRobot 格式提供关节状态/动作、末端执行器相关状态与时间信息；标定元数据也需逐任务检查。 | 有步骤、原子技能及物体 2D 框等语义标注，**区间/关键帧框不等于持续点轨迹或物体 6D**。适合真实双臂和失败恢复；先用约 7 GB 示例核对深度编码与时序。见[2026 版数据卡与字段](https://huggingface.co/datasets/agibot-world/AgiBotWorld2026)。 |
| **RoboTAP，2023 预印本 / ICRA 2024** | 真实机器人操作视频，核心优势是人工标注的 2D 点位置及可见性。 | 方法论文涉及机器人控制，但**不能据此推断公开 tracking benchmark 含完整关节/动作日志**。 | 适合夹爪遮挡、物体抓取时的 2D 跟踪评测；不是已确认带 RGB-D 和 3D 轨迹的联合几何基准。见[项目](https://robotap.github.io/)、[论文](https://arxiv.org/abs/2308.15975)、[TAP 数据与代码入口](https://github.com/google-deepmind/tapnet)。 |
| **HOPE-Video，HOPE 系列 2021–2022** | 机器人臂搭载 RealSense D415；10 段、2,038 帧 RGB-D，物体模型、相机内外参、物体位姿及重建参考。 | 发布相机轨迹；未确认包含完整机器人关节日志。相机位姿由运动学初始化后经视觉优化。 | **主要是静态物体、移动相机**，适合作为相机运动分离与尺度校验对照，不能当成动态物体操作集。`scene.ply` 是重建参考，不是无误差激光真值。使用前核查 README 的外参修正提示。见[官方数据说明](https://github.com/swtyree/hope-dataset)。 |

**三个容易影响实验结论的发布细节：**

1. **DROID 要区分 RLDS 和原始立体数据。** 官方提供约 2 GB、100 episode 的 RLDS 调试子集，但它不保证满足几何实验所需的原始双目/深度。2025 年的改进外参覆盖约 3.6 万 episode，也不等于所有轨迹均获修正。使用衍生标注时，要匹配 episode、相机、时间戳和标定版本。[官方版本说明](https://github.com/droid-dataset/droid_policy_learning)
2. **RH20T 的 RGB、depth、robot 数据分包发布。** 官方指出 640×360 压缩版深度可能受压缩影响，320×180 版深度采用无损压缩；还应检查配置对应的标定/关节补丁。因此“分辨率更高”不自动意味着深度更可信。[官方下载与修正说明](https://rh20t.github.io/)
3. **Robo360 当前可见公开包与论文采集系统的覆盖不能混写。** 数据卡列有视频、相机参数及 `poses_bounds.npy`，其中 near/far bounds 不是逐像素深度。若以它评估 depth 或 robot trajectory，必须先确认实际可下载文件含对应数据。[公开数据卡](https://huggingface.co/datasets/liuyubian/Robo360)

### 19.3 已增补 3D 点轨迹的机器人数据：最相关，但必须说明标签来源

| 资源 | 实际提供的监督 | 对本项目的价值与边界 |
|---|---|---|
| **PointWorld-DROID，2026；PointWorld 为 CVPR 2026 Highlight（项目标注）** | 对 DROID 生成深度、相机和 3D point-flow 标注。流程使用 FoundationStereo、VGGT、CoTracker3 等；数据卡列出预计算点轨迹、相机元数据、置信度及可选降采样深度。 | **当前能找到实际数据和处理代码，是有用的真实操作伪标签资源。** 约 4.2 万原始 episode 产生约 40 万个 **1 秒**片段；不能当成 40 万条完整长视频。适合适配、短程分析和案例筛选；若待测方法共享深度/跟踪教师，应披露标签依赖，避免用同源伪标签宣称独立 GT 提升。见[项目](https://point-world.github.io/)、[数据卡](https://huggingface.co/datasets/nvidia/PointWorld-DROID)、[数据处理代码](https://github.com/NVlabs/PointWorld/tree/data)。 |
| **TAPVid-MV 的 DROID 标注与 DROID50，2026-09** | 论文描述 5,371 个已标注 episode，并构建 50 episode、每个三路视角的 DROID50。静态背景轨迹依赖立体深度与几何一致性；机械臂表面轨迹由 URDF、关节运动学和几何可见性生成。 | **主要覆盖静态背景 + 机器人连杆表面，不能推断覆盖任意运动物体。** 标定/深度经过估计与校正，仍有误差。可用于相机运动与机器人运动分离；但截至本次核查，项目页的 Data/Code 仍标 `soon`，论文描述公开不等于已取得可下载标注，暂不设为短周期唯一依赖。见[论文附录 F](https://arxiv.org/html/2609.01899v1)、[项目及发布状态](https://tapvidmv.github.io/)。 |

PointWorld-DROID 的发布包以 HDF5 / JSON 等标注为主，**不是完整原始 DROID RGB 的替代品**；需要与原始视频配对，也不等于已打包好的训练 WebDataset。其 [PointWorld-BEHAVIOR](https://huggingface.co/datasets/nvidia/PointWorld-BEHAVIOR/tree/main) 则是仿真来源的 point-flow 数据，可提供更受控的几何监督；**它与下文 BEHAVIOR 2026 Challenge 的完整长时演示是不同资源**，不混用规模、时间长度和标注字段。[官方代码与数据组织](https://github.com/NVlabs/PointWorld)

### 19.4 仿真机械臂数据：更适合构建联合重建与跟踪真值

表中“可导出”表示模拟器状态支持生成该监督，**不表示下载后已经存在完整逐帧标注**。以下资源也不自动提供柔性物体的稳定材料点身份。

| 数据集 / 平台 | 视角与几何 | 机械臂和物体轨迹 | 对我们的用途与额外工作 |
|---|---|---|---|
| **CALVIN，2021 预印本 / 2022** | 固定外部及腕部 RGB-D；标准图像约为 200×200、84×84。 | `robot_obs` 含末端、夹爪与关节状态；`scene_obs` 含三个积木的 6D 位姿，以及抽屉、滑门、按钮等场景状态。 | **最适合快速搭建可控诊断。** 可同时研究机器人遮挡、刚体移动和场景关节运动；并非现成稠密点轨迹包，需要模型、相机与位姿生成点 GT。物体多样性和腕部画质有限。见[数据字段](https://github.com/mees/calvin/blob/main/dataset/README.md)、[项目](https://calvin.cs.uni-freiburg.de/)。 |
| **ManiSkill 3，2024–2025 起** | 可配置第三人称/腕部相机；支持 RGB、depth、segmentation、point cloud 与相机参数，具体视角取决于机器人和任务。 | 机器人 qpos/qvel、物体/关节状态可由模拟器获取；保存的 `env_states` 支持还原场景状态。 | **适合严格控制相机运动、遮挡与物体运动幅度。** 标准演示文件可能是 `trajectory.none…h5`，不含图像观测；需重放生成 RGB-D。优先用 `--use-env-states` 保持记录轨迹，单纯重放动作可能产生漂移。见[演示数据](https://maniskill.readthedocs.io/en/latest/user_guide/datasets/demos.html)、[重放](https://maniskill.readthedocs.io/en/latest/user_guide/datasets/replay.html)、[观测格式](https://maniskill.readthedocs.io/en/latest/user_guide/concepts/observation.html)。 |
| **RoboTwin 2.0，2025** | 双臂任务，头部/全局和腕部视角；采集器支持 RGB、毫米深度、点云和 actor/mesh 分割。 | 配置支持 `qpos`、`endpose`；模拟器内可访问物体状态，但默认导出不保证包括每个物体的完整 6D 序列。 | **适合双臂遮挡、交接、接触和域随机化。** 论文覆盖 50 个任务、5 种机器人形态。示例配置中 depth/pointcloud 可以默认关闭，需启用并检查导出；稠密点 GT 还需物体/连杆变换及可见性。见[论文](https://arxiv.org/abs/2506.18088)、[采集配置](https://robotwin-platform.github.io/doc/usage/configurations.html)。 |
| **BEHAVIOR 2026 Challenge，2026** | 移动双臂机器人，头部 720×720 与腕部 480×480 RGB-D；含 proprioception、动作和相机位姿。 | 原始 HDF5 支持 OmniGibson 状态重放；可进一步导出物体和机器人几何/运动状态。不要假设 LeRobot 包已含全部物体 6D 和点身份。 | **最值得做分钟级长期一致性。** 官方列 2 万演示、100 任务，平均约 5.9 分钟；原始 HDF5 约 1.44 TB，LeRobot V3 约 3.27 TB。2026 视觉包列 RGB+depth，深度有对数量化；追加精确几何监督宜从原始状态重放。见[2026 数据说明](https://behavior.stanford.edu/challenge/dataset.html)。 |
| **PointWorld-BEHAVIOR，2026** | 从 BEHAVIOR 仿真生成的点云/3D flow 监督，与 PointWorld 管线配套。 | 以短片段点运动为中心，适合点运动建模；不要据此假定有完整长任务的视频、所有物体 6D 或可直接重放的环境状态。 | 适合作为 PointWorld-DROID 的仿真对照或小规模适配数据；长序列问题仍应使用完整演示构造。公开文件入口见[数据集](https://huggingface.co/datasets/nvidia/PointWorld-BEHAVIOR/tree/main)、[数据准备代码](https://github.com/NVlabs/PointWorld/tree/data)。 |

**BEHAVIOR 的版本区别尤其重要：** 2025 Challenge 是 1 万演示，文档列有 RGB、depth、instance segmentation；2026 扩至 2 万演示，但当前视觉模态说明列 RGB 与 depth，不能从旧版本推断新包直接含实例分割。2026 深度使用量化编码及指定范围，评估毫米级几何前必须按说明解码并考虑量化误差。[2025 存档](https://behavior.stanford.edu/challenge/archive/2025/dataset.html)、[2026 说明](https://behavior.stanford.edu/challenge/dataset.html)

### 19.5 如何把“机器人日志”变成我们需要的联合评测

以下是建议的实验协议，**不是上述数据集已经执行或证明的结果**。

**先明确输入与监督。** 主设定仍为单路 RGB、未知相机位姿：RGB-D、多视角、机器人 qpos/末端轨迹只用于生成参考、诊断或单独的辅助输入实验。若主方法使用机器人运动学，则应明确命名为“RGB + robot proprioception”，并设置 RGB-only 对照；这会改变相机运动估计问题的难度。

**有物体 6D 位姿不等于已经有点轨迹，但对刚体可以构造：** 取物体局部坐标中的固定表面点 \(x_O\)，其世界位置为 \(x_W(t)=T_{WO}(t)x_O\)。机器人第 \(l\) 个连杆上的固定点可写为 \(x_W(t)=T_{WB}(t)\,\mathrm{FK}_l(q_t)x_l\)，再由 \(u_t=\pi(K_tT_{CW}(t)x_W(t))\) 得到图像投影。这里仍需要相机标定、统一长度单位、时间同步、准确模型、稳定物体/连杆 ID，以及深度或光线投射计算的可见性。移动基座还需要 \(T_{WB}(t)\)。物体与夹爪之间会滑动，**不能直接把末端轨迹当成物体轨迹**；布料和绳索也不能只用单一刚体位姿生成材料点真值。

建议分成三个实验层级，先用小样本验证字段和失败现象，再扩大规模：

1. **受控失败诊断：CALVIN 或 ManiSkill。** 选刚体抓取、放置、抽屉等 3–5 类任务，每类先取 10–20 段；构造静态相机/腕部相机、短遮挡/长遮挡、静止物体/搬动物体的对照。按背景、机器人表面、被操作物体分别报告深度误差、3D 点误差、可见性与遮挡后恢复率，检查“整体重建变好但动态物体身份更差”是否出现。
2. **真实场景验证：DROID 或 RH20T，加 RoboTAP/小规模人工核查。** 先匹配 20–50 段有有效深度、标定、机器人日志的完整演示，检查时间同步和投影。机器人连杆轨迹、静态背景几何与人工 2D 对应可分别验证不同部分；PointWorld 伪标签只作为补充证据。多视角重投影一致性也不是物体 3D GT 的替代品。
3. **长期累积误差：BEHAVIOR 2026。** 从少量完整任务中保留相同物理点身份，按 10 秒、30 秒、60 秒及完整序列长度测试；同时按 query age、遮挡时长和窗口边界分桶。共同测量世界坐标漂移、物体轨迹误差、机械臂/物体身份混淆以及固定内存下性能。不要在每个窗口重新对齐后宣称没有全局漂移；若单目尺度不确定，统一说明只在序列起始或全序列使用哪一种对齐协议。

关于重建评测：真实 RGB-D 通常只能评估**可见表面**，不同视角的深度融合也有传感器、标定与动态同步误差。完整表面误差、遮挡后的精确 3D 轨迹，优先在具有模型和状态的仿真数据上测；真实数据补充验证结论是否能迁移。

### 19.6 建议的实际采用顺序与证据边界

**若现在就启动实验，我建议采用“CALVIN/ManiSkill 做真值诊断 + DROID/RH20T 做真实验证 + BEHAVIOR 2026 做长序列扩展”的组合。** 有双臂交接问题时加入 RoboTwin 2.0；有软物体/动态新视角合成问题时再检查 Robo360 的可用子集。PointWorld-DROID 很适合辅助适配与筛选难例，但不作为唯一的物体轨迹评测真值。TAPVid-MV DROID50 待标注实际可取得后再加入。

开始前只需做一次小规模数据验收，避免投入数 TB 下载后才发现标注不匹配：

- **模态：** 是否有实际 RGB、可解码深度、K/外参、qpos/末端、动作？哪些只是采集过或文档可选？
- **对应：** 相机、机器人和物体是否共享时间轴、坐标系、单位？所谓 trajectory 是完整任务、滑窗片段，还是仅控制指令？
- **真值：** 物体 6D、点身份、可见性来自仿真/人工/运动学/传感器，还是其他视觉模型？是否与待测方法共享教师？
